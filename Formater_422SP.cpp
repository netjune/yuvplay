...
#define YUV_FORMATER_NAME  "422SP"

#define MODULE_VERSION    "0.0.1"
#define MODULE_AUTHOR     "netjunegg@gmail.com"
#define MODULE_COMMENT    "422sp, TI DSP上的一种格式, yyyy...uvuv..., 4:2:2"


void CFormater_422SP::_init(){
	m_buf = NULL;
	m_bufsz = 0;
...
CFormater_422SP::CFormater_422SP(){
	_init();
}
CFormater_422SP::~CFormater_422SP(){
	destroy();
...
// format name
const char *__stdcall CFormater_422SP::getName(){
	return YUV_FORMATER_NAME;
}
// other information
int __stdcall CFormater_422SP::getModuleInfo(Module_Info *pInfo){
...
int __stdcall CFormater_422SP::simpleConfig(int val, unsigned int val2, const char *str){
	return 0;
}
int __stdcall CFormater_422SP::selfConfig(){
	return 0;
}

int __stdcall CFormater_422SP::queryRuntimeInfo(char *infoBuf, int bufsz){
	return 0;
}

// 设置源数据格式, width, height, field
int __stdcall CFormater_422SP::setParams(YUV_Params *params){
	if(params == NULL)
		return E_BAD_ARG;
...
int __stdcall CFormater_422SP::update(void *pErr){
...
int __stdcall CFormater_422SP::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
...
int __stdcall CFormater_422SP::getMinBlockSize(){
	return m_block_size;
}
int __stdcall CFormater_422SP::getMinBlockFrameNum(){
	return m_frame_num;
}

// 获取buffer, buf大小应为minBlockSize
int __stdcall CFormater_422SP::getBuffer(unsigned char **ppBuf){
	if(ppBuf == NULL)
		return E_BAD_ARG;
	*ppBuf = NULL;
...
int __stdcall CFormater_422SP::putData(unsigned char *pBuf){
	if(pBuf != m_buf)
		return E_BAD_ARG;
	m_index = 0;
	return E_OK;
...
int __stdcall CFormater_422SP::getFrameIndex(){
	return m_index;
}
int __stdcall CFormater_422SP::setFrameIndex(int index){
...
int __stdcall CFormater_422SP::getFrame(int offset, unsigned char *pBuf, int bufsz){
	int index = m_index;
	if(index < 0)
		return E_NO_INIT;
	if(offset > 0) index++;
	...
	if(m_y_size != m_width*m_height || m_y_size <= 0 || bufsz <= 0 || pBuf == NULL){
		return -1;
	}
	if(m_buf == NULL || m_bufsz <= 0 || m_y_size == 0x10101010)
		return -1;
...
int __stdcall CFormater_422SP::toYourFormat(RawImage_Info *src_info, const unsigned char *src_buf, int src_bufsz, unsigned char *dst_buf, int *p_dst_bufsz){
	if(src_info == NULL)
		return -1;
...
int __stdcall CFormater_422SP::destroy(){
	if(m_buf)
		free(m_buf);
	_init();
	return 1;
...
int formater_422sp_get_interface(void **ppIntf){
	if(ppIntf == NULL)
		return -1;
	*ppIntf = new CFormater_422SP;
	return (ppIntf != NULL);
}
