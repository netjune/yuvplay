...
// format name
const char *__stdcall CFormater_I420::getName(){
	return YUV_FORMATER_NAME;
}
// other information
int __stdcall CFormater_I420::getModuleInfo(Module_Info *pInfo){
...
int __stdcall CFormater_I420::simpleConfig(int val, unsigned int val2, const char *str){
	return 0;
}
int __stdcall CFormater_I420::selfConfig(){
	return 0;
}

int __stdcall CFormater_I420::queryRuntimeInfo(char *infoBuf, int bufsz){
	return 0;
}

// 设置源数据格式, width, height, field
int __stdcall CFormater_I420::setParams(YUV_Params *params){
	if(params == NULL)
		return E_BAD_ARG;
...
int __stdcall CFormater_I420::update(void *pErr){
...
int __stdcall CFormater_I420::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
...
int __stdcall CFormater_I420::getMinBlockSize(){
	return m_block_size;
}
int __stdcall CFormater_I420::getMinBlockFrameNum(){
	return m_frame_num;
}

// 获取buffer, buf大小应为minBlockSize
int __stdcall CFormater_I420::getBuffer(unsigned char **ppBuf){
	if(ppBuf == NULL)
		return E_BAD_ARG;
	*ppBuf = NULL;
...
int __stdcall CFormater_I420::putData(unsigned char *pBuf){
	if(pBuf != m_buf)
		return E_BAD_ARG;
	m_index = 0;
	return E_OK;
...
int __stdcall CFormater_I420::getFrameIndex(){
	return m_index;
}
int __stdcall CFormater_I420::setFrameIndex(int index){
...
int __stdcall CFormater_I420::getFrame(int offset, unsigned char *pBuf, int bufsz){
	int index = m_index;
	if(index < 0)
		return E_NO_INIT;
	if(offset > 0) index++;
...
int __stdcall CFormater_I420::toYourFormat(RawImage_Info *src_info, const unsigned char *src_buf, int src_bufsz, unsigned char *dst_buf, int *p_dst_bufsz){
	if(src_info == NULL)
		return -1;
...
int __stdcall CFormater_I420::destroy(){
	if(m_buf)
		free(m_buf);
	_init();
	return 1;
