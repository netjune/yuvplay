...
// format name
const char *__stdcall CFormater_Y16::getName(){
	return YUV_FORMATER_NAME;
}
// other information
int __stdcall CFormater_Y16::getModuleInfo(Module_Info *pInfo){
...
int __stdcall CFormater_Y16::simpleConfig(int val, unsigned int val2, const char *str){
	if(val == 0)
		m_y16_endian_offset = 0;
	else
...
int __stdcall CFormater_Y16::selfConfig(){


	return 0;
}

int __stdcall CFormater_Y16::queryRuntimeInfo(char *infoBuf, int bufsz){
...
int __stdcall CFormater_Y16::setParams(YUV_Params *params){
	if(params == NULL)
		return E_BAD_ARG;
...
int __stdcall CFormater_Y16::update(void *pErr){
...
int __stdcall CFormater_Y16::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
...
int __stdcall CFormater_Y16::getMinBlockSize(){
	return m_block_size;
}
int __stdcall CFormater_Y16::getMinBlockFrameNum(){
	return m_frame_num;
}

// 获取buffer, buf大小应为minBlockSize
int __stdcall CFormater_Y16::getBuffer(unsigned char **ppBuf){
	if(ppBuf == NULL)
		return E_BAD_ARG;
	*ppBuf = NULL;
...
int __stdcall CFormater_Y16::putData(unsigned char *pBuf){
	if(pBuf != m_buf)
		return E_BAD_ARG;
	m_index = 0;
	return E_OK;
...
int __stdcall CFormater_Y16::getFrameIndex(){
	return m_index;
}
int __stdcall CFormater_Y16::setFrameIndex(int index){
...
int __stdcall CFormater_Y16::getFrame(int offset, unsigned char *pBuf, int bufsz){
	int index = m_index;
	if(index < 0)
		return E_NO_INIT;
	if(offset > 0) index++;
...
int __stdcall CFormater_Y16::toYourFormat(RawImage_Info *src_info, const unsigned char *src_buf, int src_bufsz, unsigned char *dst_buf, int *p_dst_bufsz){
	if(src_info == NULL)
		return -1;
...
int __stdcall CFormater_Y16::destroy(){
	if(m_buf)
		free(m_buf);
	_init();
	return 1;
