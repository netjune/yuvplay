...
#define YUV_FORMATER_NAME  "420SP"

#define MODULE_VERSION    "0.0.1"
#define MODULE_AUTHOR     "netjunegg@gmail.com"
#define MODULE_COMMENT    "420sp, TI DSP上的一种格式, yyyy...uvuv..., 4:1:1"


void CFormater_420SP::_init(){
	m_buf = NULL;
	m_bufsz = 0;
...
CFormater_420SP::CFormater_420SP(){
	_init();
}
CFormater_420SP::~CFormater_420SP(){
	destroy();
...
// format name
const char *__stdcall CFormater_420SP::getName(){
	return YUV_FORMATER_NAME;
}
// other information
int __stdcall CFormater_420SP::getModuleInfo(Module_Info *pInfo){
...
int __stdcall CFormater_420SP::simpleConfig(int val, unsigned int val2, const char *str){
	return 0;
}
int __stdcall CFormater_420SP::selfConfig(){
	return 0;
}

int __stdcall CFormater_420SP::queryRuntimeInfo(char *infoBuf, int bufsz){
	return 0;
}

// 设置源数据格式, width, height, field
int __stdcall CFormater_420SP::setParams(YUV_Params *params){
	if(params == NULL)
		return E_BAD_ARG;
...
int __stdcall CFormater_420SP::update(void *pErr){
...
int __stdcall CFormater_420SP::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
...
int __stdcall CFormater_420SP::getMinBlockSize(){
	return m_block_size;
}
int __stdcall CFormater_420SP::getMinBlockFrameNum(){
	return m_frame_num;
}

// 获取buffer, buf大小应为minBlockSize
int __stdcall CFormater_420SP::getBuffer(unsigned char **ppBuf){
	if(ppBuf == NULL)
		return E_BAD_ARG;
	*ppBuf = NULL;
...
int __stdcall CFormater_420SP::putData(unsigned char *pBuf){
	if(pBuf != m_buf)
		return E_BAD_ARG;
	m_index = 0;
	return E_OK;
...
int __stdcall CFormater_420SP::getFrameIndex(){
	return m_index;
}
int __stdcall CFormater_420SP::setFrameIndex(int index){
...
int __stdcall CFormater_420SP::getFrame(int offset, unsigned char *pBuf, int bufsz){
	int index = m_index;
	if(index < 0)
		return E_NO_INIT;
	if(offset > 0) index++;
...
int __stdcall CFormater_420SP::toYourFormat(RawImage_Info *src_info, const unsigned char *src_buf, int src_bufsz, unsigned char *dst_buf, int *p_dst_bufsz){
	if(src_info == NULL)
		return -1;
...
int __stdcall CFormater_420SP::destroy(){
	if(m_buf)
		free(m_buf);
	_init();
	return 1;
...
int formater_420sp_get_interface(void **ppIntf){
	if(ppIntf == NULL)
		return -1;
	*ppIntf = new CFormater_420SP;
	return (ppIntf != NULL);
}
