...
#define IMAGE_FILTER_NAME  "图像左右翻转（镜像）"
...
// filter name
const char *__stdcall CFilter_Mirror::getName(){
	return IMAGE_FILTER_NAME;
}
// other information
int __stdcall CFilter_Mirror::getModuleInfo(Module_Info *pInfo){
...
int __stdcall CFilter_Mirror::simpleConfig(int val, unsigned int val2, const char *str){
	m_testVal = val;
	m_testVal2 = val2;
...
int __stdcall CFilter_Mirror::selfConfig(){

	return E_OK;
}

int __stdcall CFilter_Mirror::queryRuntimeInfo(char *infoBuf, int bufsz){
...
int __stdcall CFilter_Mirror::setParams(RawImage_Info *pInfo){
	if(pInfo == NULL)
		return E_BAD_ARG;
...
int __stdcall CFilter_Mirror::update(void *pErr){
...
int __stdcall CFilter_Mirror::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
	*out = m_inInfo;
	return E_OK;
...
// filter
int __stdcall CFilter_Mirror::filter(unsigned char **ppData, int *pLen){
	...
	if(m_inInfo.raw_format == RAW_FORMAT_BGR24
		...
		p21 += m_inInfo.width - 1;
		p22 += m_inInfo.width - 1;
		p23 += m_inInfo.width - 1;
		...
		for(i = 0;i < m_inInfo.height;i++){
			...
			p21 += m_inInfo.width*2;
			p22 += m_inInfo.width*2;
			p23 += m_inInfo.width*2;
		}
	}
...
int __stdcall CFilter_Mirror::destroy(){
	if(m_pBuf)
		free(m_pBuf);
	m_pBuf = NULL;
	return 1;
