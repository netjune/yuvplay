...
int CYUVDoc::update(void* pErr){
	...
	if(m_pFormater->update(NULL) < 0){
		m_errMsg.Format("unsupportted yuv params 2");
		return -1;
	}
	if(m_fileSource.init(m_fileName, 0) < 0){
		m_errMsg.Format("open file \"%s\" failed",m_fileName);
		return -1;
	}
	
	int block_size = m_pFormater->getMinBlockSize();
	if(block_size <= 0){
		m_errMsg.Format("invalid block size: %d",block_size);
		return -2;
	}
	m_fileSource.setBlockSize(block_size);
	if(m_fileSource.getBlockNum() < 1){
		m_errMsg.Format("文件太小, 不足一帧");
		return -3;
	}
